...
class ImageComposition():
    ...
    def _transform_foreground(self, fg, fg_path):
        ...
        # Shift the Width
        width_random_num = random.random() * 6 + 1  # Pick something between 1 and 6
        if width_random_num > 3:
            mywidth = int(random.random() * 10 + 70)  # change width of the image: 70 - 80 pixels

            wpercent = int(mywidth / float(fg_image.size[0]))
            hsize = int((float(fg_image.size[1]) * float(wpercent)))
            if mywidth > 0 and hsize > 0:
                fg_image = fg_image.resize((mywidth, hsize), Image.ANTIALIAS)  # resize the image

        # Shift the Height
        height_random_num = random.random() * 6 + 1  # Pick something between 1 and 6
        if height_random_num > 3:
            myheight = int(random.random() * 10 + 65)  # change height of the image: 65 - 75 pixels
            
            hpercent = int(myheight / float(fg_image.size[0]))
            wsize = int((float(fg_image.size[1]) * float(hpercent)))
            if myheight > 0 and wsize > 0:
                fg_image = fg_image.resize((wsize, myheight), Image.ANTIALIAS)
        ...
        # Flip the foreground
        flip_random_num = random.random() * 10 + 1 # Pick something between 1 and 10
        if flip_random_num > 4 and flip_random_num < 8:
            fg_image = fg_image.transpose(Image.FLIP_TOP_BOTTOM) # Horizontally Flip the foreground
        elif flip_random_num < 4:
            fg_image = fg_image.transpose(Image.FLIP_LEFT_RIGHT) # Vertically Flip the foreground
